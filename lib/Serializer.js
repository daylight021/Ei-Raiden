...
const serializeMessage = async (bot, msg) => {
    ...
    if (msg.message) {
        ...
        else if (msg.type === "interactiveResponseMessage" || msg.mtype === "interactiveResponseMessage") {
            const interactiveMsg = msg.message.interactiveResponseMessage;
            if (interactiveMsg?.nativeFlowResponseMessage) {
                const flowResponse = interactiveMsg.nativeFlowResponseMessage;
                // For single_select, the params might contain the selected id
                if (flowResponse.params && flowResponse.params.length > 0) {
                    msg.text = flowResponse.params[0].toString(); // Assuming first param is the selected id
                } else {
                    msg.text = interactiveMsg.body?.text || "";
                }
            } else {
                msg.text = interactiveMsg?.body?.text || "";
            }
            msg.body = msg.text;
            console.log(`[INTERACTIVE] Response detected: "${msg.text}"`);
        }
        ...
        msg.isConvertibleMedia = /imageMessage|videoMessage/.test(msg.type) || (msg.type === 'documentMessage' && msg.msg?.mimetype && (msg.msg.mimetype.startsWith('image/') || msg.msg.mimetype.startsWith('video/')));
