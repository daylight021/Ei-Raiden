...
const RoleInfo = {
    [Roles.WEREWOLF]: { name: "Werewolf 🐺", team: "Werewolf", description: "Setiap malam, bersama werewolf lain, pilih satu warga untuk dimangsa. Balas pesan ini dengan angka targetmu (contoh: 3).", image: 'werewolf.jpg' },
    ...
    [Roles.CUPID]: { name: "Cupid 💘", team: "Villagers", description: "Pada malam pertama, pilih dua pemain untuk menjadi sepasang kekasih. Balas pesan ini dengan dua angka dipisah koma (contoh: 1,2).", image: 'cupid.jpg' }
};
...
async function progressGame(bot, groupId, usedPrefix) {
    ...
    switch (game.phase) {
        ...
        case GamePhase.NIGHT_CUPID:
            ...
            if (cupid && game.day === 1) {
                ...
                const playerList = "Balas pesan ini dengan dua angka dipisah koma (contoh: 1,2):\n" + getPlayerList(game.players.filter(p => p.isAlive));
        ...
        case GamePhase.DAY_DISCUSSION:
            ...
            if (game.hunterTrigger) {
                ...
                await bot.sendMessage(hunter.id, { text: `Kamu telah terbunuh, tapi misimu belum selesai. Pilih satu orang untuk kau bawa mati bersamamu. Balas pesan ini dengan angka targetmu (contoh: 3).\n\nDaftar target:\n${alivePlayersList}` });
...
async function handlePrivateMessage(bot, msg) {
    ...
    if (player.role === Roles.CUPID && game.phase === GamePhase.NIGHT_CUPID && game.day === 1) {
        const numbers = text.split(',').map(n => parseInt(n.trim()) - 1);
        if (numbers.length !== 2 || numbers.some(n => isNaN(n) || n < 0 || n >= game.players.length)) return msg.reply("⚠️ Kamu harus memilih DUA angka valid (contoh: 1,2).");
        const p1 = game.players[numbers[0]];
        const p2 = game.players[numbers[1]];
        if (!p1 || !p2 || !p1.isAlive || !p2.isAlive) return msg.reply("⚠️ Salah satu pemain tidak valid atau sudah mati.");
        ...
        if (isNaN(number) || number < 0 || number >= game.players.length) return msg.reply("⚠️ Kamu harus memilih SATU angka valid (contoh: 3).");
        const target = game.players[number];
