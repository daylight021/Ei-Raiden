...
function getPythonCommand() {
    ...
    for (const py of pythonVersions) {
        try {
            const { stdout } = require('child_process').execSync(`${command} ${py} ${stderrRedirect}`, { encoding: 'utf8' });
    ...
    try {
        ...
        try {
            ...
            if (fileType.includes('ELF') || fileType.includes('executable')) {
                ...
                command = `"${YT_DLP_PATH}" --cookies /root/xyzbot/cookies/cookies.txt --dump-json --no-warnings "${url}"`;
            } else {
                // Python script - use Python interpreter
                command = `${PYTHON_CMD} "${YT_DLP_PATH}" --cookies-from-browser chrome --dump-json --no-warnings "${url}"`;
            }
        } catch (e) {
            // Fallback: try direct execution first
            command = `"${YT_DLP_PATH}" --cookies-from-browser chrome --dump-json --no-warnings "${url}"`;
        }
...
async function downloadAudio(msg, bot, url, bitrate, usedPrefix, command) {
    ...
    try {
        ...
        let downloadCmd = `${ytdlpCmd} -f ${selectedAudio.format_id} -x --audio-format mp3 -o "${audioPath}" --no-warnings`;
        if (fs.existsSync('/root/xyzbot/cookies/cookies.txt')) {
            downloadCmd += ` --cookies /root/xyzbot/cookies/cookies.txt`;
        }
        downloadCmd += ` "${url}"`;
...
module.exports = {
    ...
    execute: async (msg, { args, bot, usedPrefix, command }) => {
        ...
        try {
            ...
            // Simpan URL dan message ID di session untuk user ini
            if (!global.ytmp3Sessions) global.ytmp3Sessions = {};

            const infoMessage = await msg.reply(`🎵 *${videoTitle}*\n\n` +
                `📺 *Channel:* ${channel}\n` +
                `👀 *Views:* ${viewCount}\n` +
                `👍 *Likes:* ${likes}\n` +
                `⏱️ *Duration:* ${duration}\n` +
                ...
                uniqueBitrates.slice(0, 3).map(bitrate => `• .${Math.round(bitrate)} (${Math.round(bitrate)}kbps)`).join('\n') + '\n\n' +
                `💡 *Cara:* Reply pesan ini dengan kualitas yang diinginkan (contoh: .128)`);
            ...
            console.log(`[YTMP3] Session created for ${msg.sender} with URL: ${url} and messageId: ${infoMessage.key.id}`);
